...
typedef struct {
    ...
    XPredict_ensemble_uio_map maps[ MAX_UIO_MAPS ];
} XPredict_ensemble_uio_info;

/***************** Variable Definitions **************************************/
static XPredict_ensemble_uio_info uio_info;
...
static int uio_info_read_name(XPredict_ensemble_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPredict_ensemble_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPredict_ensemble_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPredict_ensemble_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPredict_ensemble_Initialize(XPredict_ensemble *InstancePtr, const char* InstanceName) {
	XPredict_ensemble_uio_info *InfoPtr = &uio_info;
